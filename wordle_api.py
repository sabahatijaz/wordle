...
def guess_random_word(seed):
    ...
    if random_word:
        ...
        while attempts < max_attempts:
            guess_url = f'{API_URL}/guess/random?guess={guess}&seed={seed}'
            ...
            if response.status_code == 200:
                ...
                if all(slot['result'] == 'correct' for slot in feedback):
                ...
                print(f"Failed to make guess. Status code: {response.status_code}")
                break
...
def refine_guess(current_guess, feedback, target_word):
    ...
    for slot in feedback:
        if slot['result'] == 'correct':
            ...
            new_guess[slot['slot']] = next_letter(current_guess[slot['slot']])
